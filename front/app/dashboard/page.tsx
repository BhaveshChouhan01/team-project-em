...
import { MessageSquare, TrendingUp, Users, Clock, Plus, TrendingDown, User } from 'lucide-react';
...
const ChatbotDashboard: React.FC = () => {
  ...
  const handleLogout = async () => {
    try {
      // 1. Call the logout API endpoint to clear the secure cookie
      await fetch('/api/auth/logout', { method: 'POST' });

      // 2. Remove any user info from localStorage
      localStorage.removeItem('username');

      // 3. Redirect to the sign-in page using the Next.js router
      router.push('/signin');
  ...
  const maxValue = Math.max(...activityData.map(d => d.value), 1); // Avoid division by zero
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            ...
            <div className="relative" style={{ height: chartHeight + 40 }}>
              <div className="flex items-end justify-between h-full gap-8 pb-8 pl-8"> {/* Added pl-8 for Y-axis spacing */}
